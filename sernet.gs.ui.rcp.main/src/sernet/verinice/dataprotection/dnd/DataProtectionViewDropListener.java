 ...
 ******************************************************************************/
package sernet.verinice.dataprotection.dnd;
...
import sernet.verinice.iso27k.rcp.action.PasteHandler;
 ...
 *
 */
public class DataProtectionViewDropListener extends ViewerDropAdapter implements RightEnabledUserInteraction, DropPerformer {

	private TreeViewer viewer;

	private static final Logger LOG = Logger.getLogger(DataProtectionViewDropListener.class);
	...
	public DataProtectionViewDropListener(TreeViewer viewer) {
		super(viewer);
		this.viewer = viewer;
	...
	public boolean performDrop(Object data) {
		...
		if (isActive()) {
			if (firstOne instanceof BausteinUmsetzung ) {
				return dropBaustein((CnATreeElement) target, viewer, items);
			}
	...
	private boolean dropBaustein(final CnATreeElement target, Viewer viewer, final List bausteine) {
	...
	private void createBausteinUmsetzung(List toDrop, CnATreeElement target) throws CnATreeElementBuildException, CommandException {

		try {
			IProgressRunnable operation = createOperation(target, toDrop, false);
			if(operation!=null) {
