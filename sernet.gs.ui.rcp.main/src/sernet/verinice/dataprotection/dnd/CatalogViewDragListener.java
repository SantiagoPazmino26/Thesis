...
public class CatalogViewDragListener implements DragSourceListener {

    private TreeViewer viewer;

    private static final Logger LOG = Logger.getLogger(CatalogViewDragListener.class);

    public CatalogViewDragListener(TreeViewer viewer) {
        this.viewer = viewer;
    }
    ...
    public void dragStart(DragSourceEvent event) {
        ...
        for (Iterator iter = selection.iterator(); iter.hasNext();) {
            ...
            if (!(o instanceof BausteinUmsetzung || o instanceof MassnahmenUmsetzung)) {
                event.doit = false;
                return;
            }
        }
    ...
    private boolean validateDrag(DragSourceEvent event) {
        return DataProtectionModelingTransfer.getInstance().isSupportedType(event.dataType);
    }
